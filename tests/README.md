...
| Parameters | A parameter passed in an example that the tool does not have; a parameter named in a skill's `## Tool` signature that the tool does not have; a behaviour- or cost-changing parameter missing from the skill that covers it (`must_document`) |
...
The Tool-signature check exists for that reason too. `lipsync` shipped, the
auto-dubbing signature and parameter table grew a row for it, and nothing
updated `tool_surface.json` — so `hosted.dubbing` sat two releases stale with a
green build, because the only parameter check that fired needed a
`tool(param=…)` example and the signature had never counted as a claim. It does
now.
